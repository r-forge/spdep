...
# (possibly already transformed by similarity)
  ...
  stopifnot(length(grep("W", lw$style)) > 0)
  stopifnot(can.be.simmed(lw))
  if (lw$style != "W:sim") lw <- similar.listw(lw)
...
# n number of observations
  ...
  resln_3 <- lminW_3(lw, ev1=attr(lambda.n, "en"),
    zero.policy=zero.policy, control=control)
...
lminW_3 <- function(lw, ev1, zero.policy=TRUE,
  control=list(
  trace=TRUE,

...
boundary points to be a short distance from one another.
  \index{subject}{Data set!New York leukemia}
  \index{subject}{neighbours!spatial}
  \index{subject}{neighbours!spatial!polygons}
  \index{function}{poly2nb@{\code{poly2nb}, \pkg{spdep}}}
  \index{subject}{polygons!contiguous neighbours}%
  \index{subject}{polygons!contiguous neighbours!queen}%
  \index{subject}{Class!SpatialPolygons@{\code{SpatialPolygons}, \pkg{sp}}}%
  \index{subject}{Class!nb@{\code{nb}, \pkg{spdep}}}%
  \index{subject}{CRAN!spdep@\pkg{spdep}}%
...
stricter rook criterion.
  ...
  \index{function}{poly2nb@{\code{poly2nb}, \pkg{spdep}}}
  \index{subject}{polygons!contiguous neighbours}%
  \index{subject}{polygons!contiguous neighbours!rook}%
  \index{subject}{polygons!contiguous neighbours!queen}%
  \index{subject}{Class!nb@{\code{nb}, \pkg{spdep}}}%
  \index{subject}{CRAN!spdep@\pkg{spdep}}%
...
the rook relationship. Integration with GRASS was discussed in
Sect.\,\ref{sect:GRASS}, and functions in \pkg{RArcInfo} and the
equivalent \code{readOGR} function in \pkg{rgdal} for reading
ArcGIS\texttrademark~coverages in Sects.\,\ref{coverage:here} and
\ref{OGR:here}\footnote{A script to access
ArcGIS\texttrademark~coverages using Python and R(D)COM using
\code{readOGR} is on the book website.} can also be used for
retrieving rook neighbours.
  \index{subject}{CRAN!rgdal@\pkg{rgdal}}%
  \index{function}{readOGR@{\code{readOGR}, \pkg{rgdal}}}%
  \index{subject}{CRAN!RArcInfo@\pkg{RArcInfo}}%
...
diverge by distances that cannot be seen on\linepbrk* plots, but which
...
the input polygons stop at the channel banks on each side.\index{subject}{Data set!New York leukemia}%
  \index{subject}{neighbours!spatial}
  \index{subject}{neighbours!spatial!polygons}
  \index{subject}{polygons!contiguous neighbours!rook}%
  \index{subject}{polygons!clean topology}%
  \index{subject}{polygons!clean topology!snap}%
  \index{subject}{ArcGIS@{{A}rc{GIS}\texttrademark}}%
  \index{subject}{GRASS GIS}%
  \index{subject}{Class!nb@{\code{nb}, \pkg{spdep}}}%
  \index{subject}{CRAN!spdep@\pkg{spdep}}%
  \index{subject}{CRAN!spgrass6@\pkg{spgrass6}}%
  \index{function}{writeVECT6@{\code{writeVECT6}, \pkg{spgrass6}}}
  \index{function}{vect2neigh@{\code{vect2neigh}, \pkg{spgrass6}}}The
...
such as across a mountain range.
  \index{subject}{neighbours!spatial}
  \index{subject}{Methods!edit@\code{edit}}%
  \index{subject}{edit@\code{edit}|see{Methods, \code{edit}}}%
  \index{function}{edit.nb@{\code{edit.nb}, \pkg{spdep}}}
  \index{subject}{Class!nb@{\code{nb}, \pkg{spdep}}}%
...
graph measures, distance thresholds, and $k$-nearest neighbours.
  \index{subject}{neighbours!spatial}
  \index{subject}{polygons!centroid}%
  \index{subject}{points!neighbours}%
  \index{subject}{points!neighbours!distance bands}%
  \index{subject}{points!neighbours!$k$-nearest}%
  \index{subject}{points!neighbours!graph measures}%
  \index{subject}{Class!nb@{\code{nb}, \pkg{spdep}}}%
  \index{subject}{CRAN!spdep@\pkg{spdep}}%
...
neighbours were kindly contributed by Nicholas \hbox{Lewin-Koh.}}
  ...
  \index{subject}{neighbours!spatial}
  \index{subject}{neighbours!spatial!points}
  \index{subject}{points!neighbours}%
  \index{subject}{points!neighbours!graph measures}%
  \index{subject}{points!neighbours!triangulation}%
  \index{subject}{points!neighbours!sphere of influence}%
  \index{subject}{Class!nb@{\code{nb}, \pkg{spdep}}}%
  \index{subject}{CRAN!spdep@\pkg{spdep}}%
  \index{subject}{Data set!New York leukemia}
...
influence neighbours; (\textbf{c}) Gabriel graph neighbours;
(\textbf{d}) Relative graph neighbours} \label{fig:NB_graph}\vspace*{-12pt}
\end{figure}
  \index{subject}{Data set!New York leukemia}
  \index{subject}{Class!nb@{\code{nb}, \pkg{spdep}}}%
  \index{function}{tri2nb@{\code{tri2nb}, \pkg{spdep}}}
  \index{function}{graph2nb@{\code{graph2nb}, \pkg{spdep}}}
  \index{function}{soi.graph@{\code{soi.graph}, \pkg{spdep}}}
  \index{function}{gabrielneigh@{\code{gabrielneigh}, \pkg{spdep}}}
  \index{function}{relativeneigh@{\code{relativeneigh}, \pkg{spdep}}}
  \index{subject}{points!neighbours}%
  \index{subject}{neighbours!spatial!points}
  \index{subject}{points!neighbours!graph measures}%
  \index{subject}{points!neighbours!triangulation}%
  \index{subject}{points!neighbours!relative}%
  \index{subject}{points!neighbours!Gabriel}%
  \index{subject}{points!neighbours!sphere of influence}%
  \index{subject}{tripack@\pkg{tripack}|see{CRAN, \pkg{tripack}}}%
  \index{subject}{CRAN!tripack@\pkg{tripack}}%
  \index{subject}{CRAN!spdep@\pkg{spdep}}%
...
\code{sym} argument to insert links to restore\linepbrk* symmetry, but the
...
spatial weights can become block-diagonal if observations are appropriately sorted.
  ...
  \index{subject}{neighbours!spatial}
  \index{subject}{neighbours!spatial!points}
  \index{function}{is.symmetric.nb@{\code{is.symmetric.nb}, \pkg{spdep}}}
  \index{function}{n.comp.nb@{\code{n.comp.nb}, \pkg{spdep}}}
  \index{subject}{CRAN!spdep@\pkg{spdep}}%
...
coordinates.
  ...
  \index{subject}{points!neighbours}%
  \index{subject}{points!neighbours!$k$-nearest}%
  \index{function}{knn2nb@{\code{knn2nb}, \pkg{spdep}}}
  \index{function}{knearneigh@{\code{knearneigh}, \pkg{spdep}}}
  \index{subject}{Class!nb@{\code{nb}, \pkg{spdep}}}%
  \index{subject}{CRAN!spdep@\pkg{spdep}}%
  \index{subject}{Data set!New York leukemia}
...
\code{make.sym.nb} function. The $k=1$ object is also\linepbrk* useful in
...
\code{longlat} argument to handle geographical coordinates.
  ...
  \index{subject}{points!neighbours}%
  \index{subject}{points!neighbours!distance bands}%
  \index{function}{dnearneigh@{\code{dnearneigh}, \pkg{spdep}}}
  \index{function}{nbdists@{\code{nbdists}, \pkg{spdep}}}
  \index{subject}{CRAN!spdep@\pkg{spdep}}%
  \index{subject}{Data set!New York leukemia}
...
and returns a list of lists, from first to \code{maxlag} order neighbours.
  \index{subject}{Class!nb@{\code{nb}, \pkg{spdep}}}%
  \index{subject}{neighbours!spatial!higher order}
  \index{subject}{polygons!contiguous neighbours}%
  \index{subject}{points!neighbours}%
  \index{subject}{polygons!contiguous neighbours!higher order}%
  \index{subject}{points!neighbours!higher order}%
  \index{subject}{CRAN!spdep@\pkg{spdep}}%
  \index{subject}{Data set!New York leukemia}
...
type rook or queen.
  \index{subject}{Class!nb@{\code{nb}, \pkg{spdep}}}%
  \index{subject}{grid}%
  \index{subject}{grid!neighbours}%
  \index{subject}{grid!neighbours!rook}%
  \index{subject}{grid!neighbours!queen}%
  \index{subject}{neighbours!spatial}
  \index{subject}{neighbours!spatial!grid}
  \index{function}{cell2nb@{\code{cell2nb}, \pkg{spdep}}}
  \index{subject}{CRAN!spdep@\pkg{spdep}}%
...
\end{footnotesize}
  ...
  \index{subject}{Data set!Meuse bank}
...
tracts, but demanding for a 3,$103 \times 3$,103 matrix.
  ...
  \index{subject}{grid}%
  \index{subject}{grid!neighbours}%
  \index{subject}{grid!incomplete!neighbours}%
  \index{subject}{neighbours!spatial}
  \index{subject}{neighbours!spatial!grid}
  \index{subject}{points!neighbours!distance bands}%
  \index{function}{dnearneigh@{\code{dnearneigh}, \pkg{spdep}}}
  \index{function}{card@{\code{card}, \pkg{spdep}}}
  \index{subject}{CRAN!spdep@\pkg{spdep}}%
  \index{subject}{Data set!Meuse bank}

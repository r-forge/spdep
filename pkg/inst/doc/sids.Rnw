...
<<results=tex,echo=FALSE>>= 
<<afigl>>
print(spplot(nc, c("ch_pmap_low", "ch_pmap_high"), at=c(0, 0.001, 0.01, 0.05, 0.1, 1), col.regions=grey.colors(5)))
<<zfigl>>
@ 
\end{center}
...
\begin{footnotesize}
<<echo=TRUE>>= 
ecarIawll <- spautolm(ft.SID74 ~ 1, data=nc2, listw=sids.nhbr.listw.4, weights=BIR74, family="CAR", control=list(llprof=seq(-0.1, 0.9020532358, length.out=100)))
@
